...
DEF_RV_BRAND=$(toml_get "$main_config_t" rv-brand) || DEF_RV_BRAND="ReBuild"
# Force non-root APK builds only (user requested APKs only; modules disabled)
pr "Forcing non-root APK builds only (module packaging disabled). This repo now operates as 'ReBuild Build' focusing on APK outputs."
...
: >build.md
# Module update checking: prefer new config key 'enable-module-update', fall back to 'enable-magisk-update' for compatibility
ENABLE_MODULE_UPDATE=$(toml_get "$main_config_t" enable-module-update) || ENABLE_MODULE_UPDATE=$(toml_get "$main_config_t" enable-magisk-update) || ENABLE_MODULE_UPDATE=true
if [ "$ENABLE_MODULE_UPDATE" = true ] && [ -z "${GITHUB_REPOSITORY-}" ]; then
	pr "You are building locally. Module update checks will not be enabled."
...
log "\nFirst install [MicroG-RE](https://github.com/WSTxda/MicroG-RE/releases) for YouTube and Photos APKs"
